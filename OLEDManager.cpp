...
OLEDManager::OLEDManager()
    : display(SCREEN_WIDTH, SCREEN_HEIGHT, &Wire, -1), eyes(display), currentState(STATE_BOOT), previousState(STATE_ENVIRONMENT), returnState(STATE_ENVIRONMENT), stateEnterMs(0), lastAnimMs(0), lastInteractionMs(0), bootComplete(false), thankYouPhase(0), alertActive(false), alertKind(ALERT_NONE), temperature(0.0f), humidity(0.0f), lastDrawnTemp(-999.0f), lastDrawnHum(-999.0f), rtcNow(2020, 1, 1, 0, 0, 0) // ✅ RtcDateTime constructor
      ,
      lastRtcUnix(0), wifiConnected(false), lastDrawnWifi(false), wifiDotCount(0), lastWifiAnimTime(0), schedules(nullptr), scheduleCount(0), medicineLookup(nullptr), nextDose(nullptr), nextDoseValid(false), screenDirty(true), reminderBox(1)
{
    alertTitle[0] = '\0';
    alertDetail[0] = '\0';
...
void OLEDManager::triggerReminder(int box, const char *medicine)
{
    ...
    eyes.setMood(DEFAULT);
    eyes.setPosition(N);
    if (!isHighPriorityState() || currentState == STATE_SCREENSAVER ||
        currentState == STATE_ENVIRONMENT || currentState == STATE_ROBOT_EYES ||
        currentState == STATE_NEXT_MED)
    {
...
void OLEDManager::enterState(State next)
{
    ...
    if (next != STATE_ROBOT_EYES && next != STATE_SCREENSAVER)
    {
        display.clearDisplay();
        display.display();
    }
...
void OLEDManager::returnFromInterrupt()
{
    ...
    lastAnimMs = stateEnterMs;
    markDirty();
    if (currentState != STATE_ROBOT_EYES && currentState != STATE_SCREENSAVER)
    {
        display.clearDisplay();
        display.display();
    }
...
void OLEDManager::handleStateTimeouts(unsigned long now)
{
    ...
    case STATE_ROBOT_EYES:
        if (bootComplete && previousState == STATE_BOOT && elapsed >= EYES_WAKE_DURATION_MS)
            enterState(STATE_ENVIRONMENT);
        else if (bootComplete && previousState != STATE_BOOT &&
                 !isHighPriorityState() && elapsed >= EYES_DURATION_MS)
            enterState(STATE_ENVIRONMENT);
        break;
    case STATE_ENVIRONMENT:
        if (!isHighPriorityState() && elapsed >= ENV_DURATION_MS)
            enterState(STATE_ROBOT_EYES);
        break;
    case STATE_NEXT_MED:
    ...
    case STATE_THANK_YOU:
        ...
        else if (thankYouPhase == 1 && elapsed >= THANK_EYES_DURATION_MS)
        {
            thankYouPhase = 0;
            eyes.setMood(DEFAULT);
            returnFromInterrupt();
...
{
    if (isHighPriorityState() || currentState == STATE_BOOT)
        return;
...
void OLEDManager::updateEmotionalEyes()
{
    ...
    {
        eyes.setMood(DEFAULT);
        eyes.setPosition(N); // focus straight ahead
...
void OLEDManager::renderIfNeeded(unsigned long now)
{
    ...
    case STATE_ENVIRONMENT:
    {
        const bool dataChanged = (temperature != lastDrawnTemp) ||
                                 ...
                                 (rtcNow.Unix32Time() != lastRtcUnix); // ✅ Unix32Time
...
{
    display.clearDisplay();
    drawCenteredF(6, F("MedGuardian"), 2);
    drawCenteredF(26, F("Smart Medication"), 1);
    drawCenteredF(36, F("Dispenser"), 1);

    display.setTextSize(1);
    display.setCursor(34, 50);
    display.print(F("Loading"));
    const int dots = (int)((now - stateEnterMs) / BOOT_DOT_INTERVAL_MS) % 4;
    for (int i = 0; i < dots; i++)
        display.print('.');

    // Slowly open eyes halfway through boot
    if (now - stateEnterMs > BOOT_DURATION_MS / 2)
    {
        eyes.open();
    }
    display.display();
...
void OLEDManager::renderEnvironment()
{
    ...
    drawIcon(12, 44, ICON_TEMP, 12, 12);
    display.setCursor(28, 46);
    display.print(tempBuf);
    display.print(F("C"));

    // Humidity
    drawIcon(68, 44, ICON_HUM, 12, 12);
...
{
    const bool frameOn = ((now - stateEnterMs) / REMINDER_BLINK_MS) % 2 == 0;
    display.clearDisplay();
    ...
    drawCenteredF(10, F("TAKE YOUR"), 1);
    drawCenteredF(22, F("MEDICATION"), 2);
    char boxBuf[16];
    snprintf(boxBuf, sizeof(boxBuf), "Box %d", reminderBox);
    drawCenteredText(48, boxBuf, 1);
    if (reminderMedicine[0])
        drawCenteredText(58, reminderMedicine, 1);
